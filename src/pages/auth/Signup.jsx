...
import { FaHospital, FaUserDoctor, FaBellConcierge, FaIdCard, FaEnvelope, FaLock, FaEye, FaEyeSlash, FaArrowRight, FaStar, FaShieldHalved, FaUserTie, FaGoogle } from 'react-icons/fa6'
...
export default function Signup() {
  ...
  const handleGoogleSignup = async () => {
    if (!selectedRole) {
      setErrors(prev => ({ ...prev, role: 'Please select a professional role first' }))
      return
    }
    ...
    try {
      ...
      if (error.message && (error.message.includes('Firestore permission') || error.message.includes('permission'))) {
        setErrors(prev => ({ ...prev, general: error.message }))
      } else if (error.code === 'auth/popup-closed-by-user') {
        setErrors(prev => ({ ...prev, general: 'The Google Sign-In popup was closed before completion.' }))
      } else {
        setErrors(prev => ({ ...prev, general: 'Failed to sign up with Google. Please try again.' }))
      }
      setIsLoading(false)
    }
  }
          ...
          <div className="backdrop-blur-xl bg-white/5 border border-white/10 rounded-3xl p-8 shadow-2xl shadow-black/20">
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <button 
                ...
                disabled={isLoading}
                className="w-full py-4 px-6 border-2 border-white/10 bg-white/5 hover:bg-white/10 text-white font-bold text-lg rounded-2xl shadow-lg hover:shadow-xl hover:border-white/20 transition-all duration-300 transform hover:scale-105 flex items-center justify-center space-x-2 cursor-pointer disabled:cursor-not-allowed"
              >
